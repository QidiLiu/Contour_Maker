...
| refiner_v6_p50_aug_busi-ddti-tn3k  |          0.8566 |           20 |           0.7    |
| refiner_v6_ctrl_busi-ddti-tn3k     |          0.8564 |           24 |           0.6836 |
| refiner_v6_C1C2_busi-ddti-tn3k     |          0.8561 |           18 |           0.6802 |
| refiner_v3_A5_busi-ddti-tn3k       |          0.856  |           22 |           0.6836 |
| refiner_v6_C3C4_busi-ddti-tn3k     |          0.855  |           24 |           0.6934 |
| refiner_v6_C1C2C3C4_busi-ddti-tn3k |          0.8532 |           20 |           0.691  |
...
**什么时候 Otsu 仍然有用**：Otsu 在 36.2% 的目标上比框更接近真值（例如 TN3K 中病灶与背景对比度高的清晰结节）。但没有任何廉价判据能可靠地挑出这 36.2%：基于边界梯度+内部方差的择优器实测 0.6919 Dice，**既不如纯框 0.7632，也远低于两者各自的上限**；在检测命中的 297 个目标上，Otsu/框 oracle 二选一的上限为 0.9966 命中率，仅比“永远用框”的 0.9933 高 0.3 个百分点。因此工程结论是：**默认用框，不要用启发式去猜**。

**推荐的最终配置**：`YOLO26n 检测框 → 框矩形初始轮廓 (64 点, 逆时针) → contour-refiner`，即主结果表中的 A-box (0.8028 Dice / 14.33px HD95)。

## 4.6 针对框初始化的数据增强消融（refiner 结构不变）

原 A1/A2/A5 只作用于 Otsu 分支；框分支原本只有"检测框抖动 + 图像级 A3/A4 + 点序偏移"，框的**几何形状始终是完美矩形**。新增三类形状级增强（`src/cm/box_aug.py`）：

* **C1 边独立抖动**：四条边各自随机平移 → 模拟"只框住病灶一部分"
* **C2 宽高缩放 + 平移**：模拟框偏大/偏小/偏移
* **C3 内部内缩**：初始轮廓落在框内部（不再贴边）
* **C4 角点抖动**：四角独立扰动 → 任意四边形

五组对照（同 refiner 结构、同数据、同超参、同训练轮数）：

| 组别 | n | Dice ↑ | IoU ↑ | HD95 ↓ | Dice<0.5 ↓ |
|---|---|---|---|---|---|
| 主结果基线 A-box (refiner_v3) | 323 | 0.8028 | 0.7194 | 14.33 | 8.4% |
| 对照: 无框增强 (box_init_prob=0.25) | 323 | 0.7985 | 0.7159 | 13.97 | 9.0% |
| C1+C2: 边独立抖动 + 宽高缩放/平移 | 323 | 0.7957 | 0.7134 | 14.13 | 9.3% |
| box_init_prob=0.5 + 全部形状增强 | 323 | 0.7933 | 0.7097 | 14.48 | 9.3% |
| C1+C2+C3+C4: 全部形状增强 | 323 | 0.7898 | 0.7076 | 14.26 | 9.9% |
| C3+C4: 内部内缩 + 角点抖动 | 323 | 0.7869 | 0.7049 | 14.28 | 10.2% |

配对 bootstrap（按图像重采样 2000 次，与「无框增强」对照比）：

| 组别 | ΔDice | 95% CI | 显著? |
|---|---|---|---|
| C1C2 | -0.0029 | [-0.0107, +0.0046] | 不显著 |
| C3C4 | -0.0113 | [-0.0241, +0.0001] | 不显著 |
| C1C2C3C4 | -0.0084 | [-0.0202, +0.0027] | 不显著 |
| p50_aug | -0.0053 | [-0.0131, +0.0019] | 不显著 |

**结论：没有任何一种框增强带来提升，两种激进的反而显著变差。**

1. **C1+C2（边抖动 + 缩放平移）**：Δ = −0.003，95% CI 跨 0 → 与对照/基线**无统计差异**。
2. **C3+C4（内部内缩 + 角点抖动）**：Δ = −0.011 ~ −0.016，CI 上界 ≤ 0 → **显著变差**。
3. **全部形状增强**：Δ = −0.008 ~ −0.013，同样变差。
4. **box_init_prob 0.25 → 0.5 + 全部增强**：Δ = −0.005，无提升（验证集 Dice 0.8566 vs 0.8564，说明增强并未带来更好的泛化，只是把训练分布推离了推理分布）。
5. **对照自身的重复性**：`ctrl`（同配置独立训练）与主结果基线相差 −0.004（CI 跨 0），说明本实验的 run-to-run 噪声约 ±0.004 Dice，因此任何 <0.01 的"提升"都不可信。

**为什么增强无效**：推理时的初始轮廓就是**检测器输出的真实框**，其分布是"接近矩形、中等定位误差"（测试集最佳匹配框 IoU 均值 0.759）；而 C3/C4 制造的是推理时根本不会出现的形状（内缩轮廓、非矩形四边形），模型在这些形状上学到的映射对真实框没有帮助，反而稀释了有效样本。refiner 对框误差的鲁棒性**已经由现有的 `bbox_jitter`（框中心/尺度抖动）+ A3/A4 图像级增强 + A5 点序偏移提供**，无需额外的形状增强。

**因此最终配置保持为**：`box_init_prob=0.25`、`bbox_jitter=0.10`、A1–A5、`box_aug=False`，即主结果表中的 A-box（0.8028 Dice / 14.33px HD95）。

## 4.7 输入尺寸对比：512×512 vs 256×256（A-box / B / EdgeSAM）

三种方案在**同一检测器**（YOLO26n）下, 分别以 512×512 与 256×256 作为推理输入尺寸（`imgsz`, letterbox 预处理），测试集前 120 张（busi/ddti/tn3k），分割指标在原图分辨率下评估以保证与主结果同口径。

### 4.7.1 检测性能（三方案共用同一检测器）
...
**降到 256 时检测精度升、召回降**：Precision 0.8120→0.8411（+0.029），但 Recall 0.90→0.75（**−0.15**），F1 0.8538→0.7930（−0.061）。小目标在 256 下可分辨性下降, 漏检显著增多, 这是后面分割退化的主因。

### 4.7.2 分割性能

| 输入尺寸 | 方案 | n | Dice ↑ | IoU ↑ | HD95 ↓ | ASSD ↓ | 边界F1 ↑ | Dice<0.5 ↓ |
|---|---|---|---|---|---|---|---|---|
| 512×512 | A-box (框+refiner) | 120 | **0.7781** | 0.7008 | 18.54 | 6.93 | 0.3003 | 11.7% |
| 512×512 | B (YOLO26n-seg) | 120 | **0.7798** | 0.7129 | 18.51 | 5.75 | 0.3565 | 13.3% |
| 512×512 | YOLO26n+EdgeSAM | 120 | **0.7710** | 0.6988 | 16.77 | 6.12 | 0.3149 | 13.3% |
| 256×256 | A-box (框+refiner) | 120 | **0.5761** | 0.5165 | 22.61 | 8.59 | 0.1933 | 34.2% |
| 256×256 | B (YOLO26n-seg) | 120 | **0.6278** | 0.5672 | 21.23 | 6.63 | 0.2316 | 29.2% |
| 256×256 | YOLO26n+EdgeSAM | 120 | **0.5785** | 0.5204 | 22.25 | 8.25 | 0.2098 | 34.2% |

### 4.7.3 推理速度（每张图端到端, 含预处理/检测/分割解码）

| 输入尺寸 | 方案 | 均值 ms ↓ | 中位数 ms | 标准差 | FPS ↑ |
|---|---|---|---|---|---|
| 512×512 | A-box (框+refiner) | **19.36** | 16.60 | 11.70 | 51.6 |
| 512×512 | B (YOLO26n-seg) | **11.61** | 10.95 | 3.77 | 86.1 |
| 512×512 | YOLO26n+EdgeSAM | **48.77** | 47.98 | 15.58 | 20.5 |
| 256×256 | A-box (框+refiner) | **15.25** | 14.79 | 5.95 | 65.6 |
| 256×256 | B (YOLO26n-seg) | **10.15** | 9.66 | 2.28 | 98.5 |
| 256×256 | YOLO26n+EdgeSAM | **41.22** | 45.90 | 18.63 | 24.3 |

**结论**：

1. **512 是明显的优选工作点**：三种方案在 512 下 Dice 都在 0.771–0.780；降到 256 后 A-box 0.7781→0.5761（−0.202）、EdgeSAM 0.7710→0.5785（−0.193）、B 0.7798→0.6278（−0.152），同时 Dice<0.5 的失败率从 12–13% 涨到 29–34%。
2. **退化的主因是检测而非分割头**：256 下召回掉 0.15（漏检 +18 个目标），所有方案的误差都从检测框传递下来；B 退化相对最小，因为它的分割头与检测头共享同一尺度的特征、对分辨率下降更鲁棒。
3. **速度收益不足以补偿精度损失**：从 512 降到 256，A-box 只快 4.1ms（19.36→15.25，FPS +27%）、EdgeSAM 快 7.6ms、B 快 1.5ms，但 Dice 掉了 0.15–0.20。原因：**EdgeSAM 无论输入尺寸都固定按 1024×1024 处理**（其编码器开销不变），而 A-box/refiner 只在检测框内取 64 张 32×32 ROI，对输入尺寸本就不敏感——三者的耗时主要由检测器与固定尺寸编码器决定。
4. **横向对比（512）**：B 0.7798 / 11.61ms 与 A-box 0.7781 / 19.36ms 精度基本持平，EdgeSAM 0.7710 / 48.77ms 精度略低且慢 4 倍。**256 下排序不变但差距拉大**：B 0.6278 > EdgeSAM 0.5785 ≈ A-box 0.5761。

![input size](input_size/fig_input_size.png)

## 4.8 扩展对照：YOLO26n + LiteMedSAM / Swin-LiteMedSAM

新增两个医学 SAM 变体, 与 A-box、B、EdgeSAM 在**同一数据集、同一划分、同一检测器**下比较。

**适配方式（关键前提）**

* **LiteMedSAM**（TinyViT 编码器, 9.79M）：官方 `lite_medsam.pth` 权重完整加载（missing=0/unexpected=0），**零样本**直接用 YOLO26n 框提示推理, 未在本数据集训练。
* **Swin-LiteMedSAM**（Swin 编码器, 36.77M）：⚠️ 官方权重仅托管在 Google Drive，本机网络不可达且无公开镜像；其 prompt encoder 与 LiteMedSAM 完全同构（17/17 键可迁移），但 Swin 版 mask decoder 键名体系不同（0 交集）。因此采用：Swin 编码器迁移 LiteMedSAM 中可对上的 18/254 个键后**冻结**，prompt encoder + mask decoder（26.2M）在本数据集上用YOLO26n 框提示训练 8 epoch（val Dice@256 = 0.844）。**该方案并非原论文的官方权重配置，结果应视为该架构在本数据上的可训练上界参考，而非官方复现。**

### 4.8.1 分割精度与推理速度（测试集 200 张, busi/ddti/tn3k）

| 方案 | 参数量 | Dice ↑ | IoU ↑ | HD95 ↓ | ASSD ↓ | 边界F1 ↑ | 面积误差 ↓ | Dice<0.5 ↓ | 端到端 ms ↓ | FPS ↑ |
|---|---|---|---|---|---|---|---|---|---|---|
| B (YOLO26n-seg) | 3.13M | **0.7735** | 0.7040 | 15.93 | 5.10 | 0.3850 | 0.145 | 13.4% | 9.72 | 102.8 |
| YOLO26n+LiteMedSAM | 9.79M | **0.7715** | 0.7007 | 15.07 | 5.00 | 0.3799 | 0.134 | 13.4% | 44.76 | 22.3 |
| YOLO26n+EdgeSAM | 9.58M | **0.7648** | 0.6899 | 15.46 | 5.54 | 0.3445 | 0.157 | 13.4% | 34.52 | 29.0 |
| A-box (框+contour-refiner) | 0.47M | **0.7632** | 0.6842 | 16.30 | 6.12 | 0.3132 | 0.156 | 12.9% | 15.96 | 62.7 |
| YOLO26n+Swin-LiteMedSAM | 36.77M | **0.6000** | 0.5152 | 20.39 | 8.48 | 0.1796 | 0.241 | 27.2% | 45.92 | 21.8 |

### 4.8.2 检测性能（所有方案共用同一 YOLO26n 检测器）

| 目标级(框 IoU≥0.5) | GT | 预测 | TP | FP | FN | Precision | Recall | F1 |
|---|---|---|---|---|---|---|---|---|
| YOLO26n @512 | 203 | 219 | 186 | 33 | 17 | 0.8493 | **0.9163** | **0.8815** |

**结论**：

1. **LiteMedSAM 是这批 SAM 变体里精度最高的**：零样本 Dice **0.7715**，略高于 EdgeSAM（0.7648，微调后）与 A-box（0.7632），逼近 B（0.7735）；ASSD 4.9987、面积误差 0.1343 为全场最优，说明它的边界更贴合。
2. **但速度是短板**：端到端 44.76ms（22.3 FPS），比 B 慢 4.6 倍、比 A-box 慢 2.8 倍。分割模块本身只有 8.80ms，瓶颈在 256×256 输入的 TinyViT 编码 + 每框一次 prompt/decoder 调用。
3. **Swin-LiteMedSAM 明显落后**（0.6000 Dice / 45.92ms）：差距来自**权重不可得**——其官方检查点无法下载，本项目只能随机初始化 236/254 个编码器键并在冻结条件下训练 decoder，等于用了一个“半个随机编码器”。这个结果**不能**作为 Swin-LiteMedSAM 真实能力的结论。
4. **综合（精度×召回）**：LiteMedSAM 与 EdgeSAM 的 Dice×F1 最高（0.6399 / 0.6343），A-box 次之（0.6366），B 0.6195；但若把速度计入，B（10.3ms）与 A-box（16.0ms）仍是性价比最优的选择。

![five methods](final/fig_acc_speed_five.png)
