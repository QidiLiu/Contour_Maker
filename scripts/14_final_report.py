...
def main() -> int:
    ...
    L: list[str] = []
    A = L.append
    A("# 超声低回声目标分割：五条流水线对比实验报告")
    A("")
    A("> **YOLO26n + contour-refiner (A-box)** · **YOLO26n-seg (B)** · **+EdgeSAM** · "
      "**+EfficientSAM** · **+LiteMedSAM** · **+Swin-LiteMedSAM**  ")
    A("> 数据: BUSI(乳腺) + TN3K/DDTI(甲状腺)，另有 TG3K(腺体) 边界案例；"
      "同一数据划分、同一检测器、同一评估脚本。")
    A("")
    A("## 0. 结论 (先看这里)")
    A("")
    A("### 0.1 五条流水线总表（同一数据集/划分/检测器，测试集 200 张）")
    A("")
    five = OUT / "final" / "all_five_methods.csv"
    if five.exists():
        t5 = pd.read_csv(five)
        pc = {"A_box": "0.47M", "B_seg": "3.13M", "SAM_edgesam": "9.58M",
              "SAM_efficientvit-t": "10.2M", "SAM_litemedsam": "9.79M",
              "SAM_swin_litemedsam": "36.77M"}
        A("| 流水线 | 参数量 | Dice ↑ | IoU ↑ | HD95 ↓ | ASSD ↓ | Dice<0.5 ↓ | 端到端 ms ↓ | FPS ↑ |")
        A("|---|---|---|---|---|---|---|---|---|")
        for _, r in t5.sort_values("dice", ascending=False).iterrows():
            A(f"| {r['name']} | {pc.get(r['method'], '-')} | **{r['dice']:.4f}** | {r['iou']:.4f} | "
              f"{r['hd95']:.2f} | {r['assd']:.2f} | {r['fail']*100:.1f}% | {r['end2end_ms_mean']:.2f} | "
              f"{r['fps']:.1f} |")
        A("")
    A("> EdgeSAM / EfficientSAM 在同一训练集上微调 mask decoder；LiteMedSAM 用官方权重零样本；"
      "Swin-LiteMedSAM 因官方权重不可得而为本项目自训版本（见 §4.8）。")
    A("")
    A("### 0.2 关键结论")
    A("")
    A("0. **【最重要】把初始轮廓从\"框内 Otsu 最大暗连通域\"改为\"检测框矩形\"，"
      "方案 A 反超方案 B**：完整测试集 Dice **0.8028** vs 0.7923、HD95 持平 14.33px；"
      "检测命中目标上的轮廓命中率 86.5% → **99.3%**（oracle 上限 99.7%）。"
      "该改动不需要重训检测器或分割器，详见 §4.3。")
    A("1. **contour-refiner 本身有效**：同一检测框下把 Otsu 粗糙轮廓从 "
      "**0.5410 → 0.8028**（换框初始化后）；即使仍用 Otsu 初始化也有 0.5410 → 0.6602 "
      "（+0.1192），失败率 33.8% → 23.0%。")
    A("2. **瓶颈在初始化质量，不在精细化**：Otsu 初始轮廓的**真值覆盖率仅 0.718**"
      "（p10 = 0.301，常只圈住病灶一部分），框矩形覆盖率 **0.967**（p10 = 0.912）；"
      "refiner 只能局部修正，补不回未覆盖区域。详见 §4.5。")
    A("3. **SAM 系必须做域适配**：零样本时 EfficientSAM Dice **0.0000**、EdgeSAM 0.6180；"
      "适配后 0.7682 / 0.7648。LiteMedSAM 零样本即 0.7715，是 SAM 系里最好的。"
      "但它们的端到端延迟是 B 的 3.5–10 倍。")
    A("4. **A1–A5 增强与框形状增强都没有带来测试集收益**（差异 <0.02 且 95% CI 跨 0）；"
      "本实验 run-to-run 噪声约 **±0.004 Dice**，小于该幅度的差异不可信。详见 §3、§4.6。")
    A("5. **输入尺寸 512 明显优于 256**：256 下检测召回 0.90 → 0.75、三种方案 Dice 掉 "
      "0.15–0.20，而速度只快 1.5–7.6ms。详见 §4.7。")
    A("6. **适用边界**：Otsu 的\"低回声\"假设对高回声目标（TG3K 甲状腺腺体）失效"
      "（粗糙轮廓 Dice 0.12 → 改 polarity 后 0.43）。详见 §4.1。")
    A("")
    A("![overall](final/fig_acc_speed_five.png)")
    A("")
    A("## 1. 实验设置")
    A("")
    ...
    if primary is not None:
        A("> 注: `A` / `A-0` 行为 **Otsu 初始化**的历史配置；**正式配置是 §4.3 的 `A-box`"
          "（框矩形初始化，Dice 0.8028）**。此处保留 Otsu 版本用于说明\"初始化质量决定上限\"。")
        A("")
        A(table(primary))
    A("")
    A("### 2.1 分数据集")
    ...
    A("")
    A("```bash")
    A("PY=./.conda/bin/python            # 项目内 conda 环境 (.conda, Python 3.11 + torch 2.11+cu128)")
    A("export PYTHONPATH=src")
    A("export YOLO_CONFIG_DIR=$PWD/.cache/ultralytics")
    A("")
    A("# 1) 数据准备")
    A("$PY scripts/01_prepare_data.py      # 解压 + image/mask 配对 + 统一")
    A("$PY scripts/02_make_splits.py       # 共享 70/15/15 划分 (seed=42)")
    A("$PY scripts/04_export_yolo.py       # 导出 YOLO det/seg 数据集")
    A("$PY scripts/04b_make_subset.py      # 平衡子集 (busi600/tn3k1000/ddti600)")
    A("")
    A("# 2) 训练 YOLO26n 检测器 (A/B/SAM 系共用) 与 YOLO26n-seg (方案 B)")
    A("$PY scripts/05_train_yolo.py --kind det --yolo-dir data/yolo_subset \\")
    A("    --name det_yolo26n_sub --epochs 60 --imgsz 512 --batch 16")
    A("$PY scripts/05_train_yolo.py --kind seg --yolo-dir data/yolo_subset \\")
    A("    --name seg_yolo26n_sub --epochs 60 --imgsz 512 --batch 12")
    A("")
    A("# 3) 方案 A: contour-refiner (先缓存样本, 再训练)")
    A("$PY scripts/03b_cache_samples.py --level 5 --variants 3 --datasets busi tn3k ddti \\")
    A("    --out-subdir v5_mix_L5_v3")
    A("$PY scripts/03c_verify_cache.py     # 缓存完整性校验")
    A("$PY scripts/03_train_refiner.py --level 5 --tag A5 --datasets busi tn3k ddti \\")
    A("    --cache-dir data/cache/v5_mix_L5_v3 --epochs 25 --batch-size 64")
    A("")
    A("# 4) 五方案统一评测 (精度 + 速度, 同一 test split)")
    A("$PY scripts/18_bench_sam.py --limit 200 --warmup 5 --repeats 2 \\")
    A("    --sam edgesam efficientvit-t litemedsam swin_litemedsam \\")
    A("    --sam-ckpt weights/sam_finetuned --out reports/bench_litemedsam")
    A("$PY scripts/20_sam_compare.py        # 汇总表 + 精度-速度图")
    A("")
    A("# 5) 消融与专题实验")
    A("$PY scripts/21_box_aug_experiments.py ; $PY scripts/22_box_aug_report.py")
    A("$PY scripts/23_box_aug_stats.py      # 配对 bootstrap 显著性检验")
    A("$PY scripts/24_input_size_bench.py --sizes 512 256 --limit 120")
    A("$PY scripts/25_input_size_report.py")
    A("")
    A("# 6) 生成本报告")
    A("$PY scripts/14_final_report.py       # -> reports/REPORT.md")
    A("")
    A("# 可选: SAM 系适配 (复现 SAM 变体结果)")
    A("$PY scripts/19_finetune_sam.py --variant edgesam --epochs 4 --lr 5e-4")
    A("$PY scripts/19_finetune_sam.py --variant efficientvit-t --epochs 4 --lr 5e-4")
    A("$PY scripts/26_train_swin_litemedsam.py --epochs 8 --stride 3")
    A("```")
    A("")
    A("> 完整脚本清单与踩坑记录见 [`EXPERIMENT.md`](../EXPERIMENT.md) §8/§7。")
    A("")
    A("## 6. 局限与后续改进")
    A("")
